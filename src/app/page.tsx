...
export default function Dashboard() {
  ...
  useEffect(() => {
    (window as unknown as { oasisWatchFlight?: (f: WatchedFlight) => void }).oasisWatchFlight = (f) => {
      if (!f?.icao24) return;
  ...
  const [oasisTheme, setOasisTheme] = useState<'core'|'ghost'>('core');


  useEffect(() => {
    document.body.className = oasisTheme === 'core' ? '' : `theme-${oasisTheme}`;
  }, [oasisTheme]);
  ...
  const handleMouseCoords = useCallback((coords: { lat: number; lng: number }) => {
    ...
    geocodeTimer.current = setTimeout(async () => {
      ...
      } catch (e) { console.warn('[OASIS] Suppressed error:', e instanceof Error ? e.message : e); }
  ...
  // ── Drawing / AOI ──
  // OasisMap already owns the draw interaction and the polygon rendering;
  ...
  const fetchEndpoint = useCallback(async (
    ...
    try {
      ...
      console.warn('[OASIS] Suppressed error:', e instanceof Error ? e.message : e);
  ...
  // ── OASIS VISION SDK — Intelligence Fusion Layer ──
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
        ...
        {showSplash && (
          ...
          >
            ...
            <div className="relative w-40 h-40 mb-8 flex items-center justify-center z-[2]">
              ...
              >
                {/* The OASIS AI mark sits inside the radar ring, so the brand is
                    what the operator sees while the layers load. */}
                <motion.img
                  src="/oasis-tree-128.png"
                  alt="OASIS AI"
                  animate={{ opacity: [0.72, 1, 0.72] }}
                  transition={{ duration: 2.4, repeat: Infinity, ease: 'easeInOut' }}
                  className="w-16 h-16 object-contain drop-shadow-[0_0_16px_rgba(124,214,255,0.55)]"
                />
                {/* Crosshair lines */}
            ...
            {/* ── OASIS VISION title — letter-by-letter stagger ── */}
            {/* Twice the letters of the old six-char mark, so the tracking is tighter
                and the row is allowed to wrap rather than overflow a phone screen. */}
            <div className="flex flex-wrap items-center justify-center gap-[2px] mb-3 z-[2] max-w-full px-4">
              {'OASIS VISION'.split('').map((letter, i) => (
                <motion.span
                  key={i}
                  initial={{ opacity: 0, y: 20, filter: 'blur(8px)' }}
                  animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                  transition={{ delay: 0.5 + i * 0.08, duration: 0.5, ease: 'easeOut' }}
                  className="text-2xl md:text-4xl font-bold tracking-[0.3em] font-mono"
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] text-[#D4AF37] font-mono">OASIS VISION</h1>
            <span className="text-[9px] md:text-[10px] font-mono tracking-[0.2em] opacity-80 uppercase text-[#D4AF37]">GLOBAL INTELLIGENCE · OASIS AI</span>
          </div>
        </div>
        <div className="flex items-center gap-3 mt-1.5 pl-[44px] min-w-0 pr-4">
          <span className="text-[9px] md:text-[9px] text-[var(--text-secondary)] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-80 truncate">
      ...
      {showLayers && !isMobile && <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} theme={oasisTheme} setTheme={setOasisTheme} capabilities={capabilities} />}
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#111] border-b border-[var(--border-primary)]">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#FF4081] animate-oasis-pulse" />
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[10px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'OASIS RECON' : mobilePanel === 'remote' ? 'WORLD REMOTE' : 'SEARCH'}
                    </span>
                  ...
                  {mobilePanel === 'layers' && (
                    <>
                      ...
                      <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} isMobile={true} theme={oasisTheme} setTheme={setOasisTheme} capabilities={capabilities} />
                      <div className="mt-8">
